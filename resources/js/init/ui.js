class Laravolt {
    static init(root) {
        ...
        $.fn.destroyDropdown = function () {
            return $(this).each(function () {
                $(this)
                    .parent()
                    .dropdown('destroy')
                    .replaceWith($(this).addClass($(this).data('class')))
            })
        }
                        ...
                        if (!value) {
                            ...
                            child.api({
                                ...
                                onError: function (errorMessage, element, xhr) {
                                    if (
                                        typeof xhr.responseJSON.exception !==
                                        'undefined'
                                    ) {
                                        alert(
                                        ...
                                        alert(
                                            'Something goes wrong with DropdownDB, but APP_DEBUG off. See application log (usually in storage/logs/laravel.log) for complete error message.'
                                        )
                                    }
                                },
                            })
        ...
        if (
            typeof AutoNumeric === 'function' &&
            $('input[data-role="rupiah"]').length > 0
        ) {
...
const TURBOLINK_ENABLED =
    $('meta[name="turbolinks-enabled"]').attr('content') === '1'

if (TURBOLINK_ENABLED) {
    let Turbolinks = window.Turbolinks
    Turbolinks.start()
    ...
    $(document).on('turbolinks:render', function (event) {
        $('#sidebar .simplebar-scroll-content').scrollTop(
            $('#sidebar').data('scroll')
        )
    })
} else {
